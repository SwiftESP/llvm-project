...
void Clang::AddXtensaTargetArgs(const ArgList &Args, ArgStringList &CmdArgs) const {
  const Driver &D = getToolChain().getDriver();
  ...
  if (Args.getLastArg(options::OPT_mfix_esp32_psram_cache_issue) != nullptr) {
    ...
    if (Arg *A = Args.getLastArg(options::OPT_mfix_esp32_psram_cache_strategy_EQ)) {
      StringRef Value = A->getValue();
      if (Value == "memw" || Value == "nops") {
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back(Args.MakeArgString("-mfix-esp32-psram-cache-strategy-" + Value));
      } else {
        D.Diag(diag::err_drv_unsupported_option_argument) << A->getOption().getName() << Value;
      }
    }
  }
